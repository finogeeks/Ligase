...
func (s *KeyUpdateRpcConsumer) cb(msg *nats.Msg) {
...
func (s *KeyUpdateRpcConsumer) startWorker(msgChan chan *types.KeyUpdateContent) {
	for data := range msgChan {
		s.processKeyUpdate(data)
	}
}

func (s *KeyUpdateRpcConsumer) Start() error {
	s.msgChan = make([]chan *types.KeyUpdateContent, s.chanSize)
	for i := uint32(0); i < s.chanSize; i++ {
		s.msgChan[i] = make(chan *types.KeyUpdateContent, 512)
...
func (s *KeyUpdateRpcConsumer) processKeyUpdate(data *types.KeyUpdateContent) {
	switch data.Type {
