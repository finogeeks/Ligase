...
func CheckAndProcessInvite(
	...
	lookupRes, storeInviteRes, err := queryIDServer(ctx, cache, cfg, userID, body, roomID, complexCache)
	if err != nil {
		return
	}
	...
	if lookupRes.MXID == "" {
		...
		// storeInviteRes.
		err = emit3PIDInviteEvent(ctx, body, storeInviteRes, userID, roomID, cfg, rsRpcCli, idg)
...
func queryIDServer(
	ctx context.Context,
	cache service.Cache, cfg config.Dendrite, userID string,
	body *MembershipRequest, roomID string, complexCache *common.ComplexCache,
	...
	if lookupRes.MXID == "" {
		...
		storeInviteRes, err = queryIDServerStoreInvite(ctx, cache, cfg, userID, body, roomID, complexCache)
		return
	}
	...
	if lookupRes.NotBefore > now || now > lookupRes.NotAfter {
		...
		return queryIDServer(ctx, cache, cfg, userID, body, roomID, complexCache)
	}
...
func queryIDServerStoreInvite(
	...
	serverName, _ := common.DomainFromID(userID)
...
func emit3PIDInviteEvent(
	...
	userID, roomID string, cfg config.Dendrite,
	...
	event, err := common.BuildEvent(builder, domainID, cfg, idg)
	if err != nil {
		return err
	}
