...
func (r *Client) InputRoomEvents(ctx context.Context, req *roomserverapi.RawEvent) (*roomserverapi.InputRoomEventsResponse, error) {
	...
	data, err := r.cli.Request(r.cfg.Rpc.RoomInputTopic, bytes, 3000+len(req.BulkEvents.Events)*1000)
	if err != nil {
		return nil, err
	}
...
func (r *Client) SendEduToRemote(ctx context.Context, req *gomatrixserverlib.EDU) error {
	...
	r.cli.Pub(types.EduTopicDef, bytes)
	return nil
}
...
func (r *Client) GetAliasRoomIDFromRemote(ctx context.Context, req *external.GetDirectoryRoomAliasRequest, targetDomain string) (*external.GetDirectoryRoomAliasResponse, error) {
	data, err := FedRpcRequest(r.cli, targetDomain, r.cfg.Rpc.FedAliasTopic, req)
	if err != nil {
		log.Infof("federation rpc request failed, err: %v", err)
		return nil, err
	}

	var response external.GetDirectoryRoomAliasResponse
...
func (r *Client) GetProfileFromRemote(ctx context.Context, req *external.GetProfileRequest, targetDomain string) (*external.GetProfileResponse, error) {
	data, err := FedRpcRequest(r.cli, targetDomain, r.cfg.Rpc.FedProfileTopic, req)
	if err != nil {
		log.Infof("federation rpc request failed, err: %v", err)
		return nil, err
	}

	var response external.GetProfileResponse
	json.Unmarshal(data, &response)
	return &response, nil
}
...
func (r *Client) GetAvatarFromRemote(ctx context.Context, req *external.GetProfileRequest, targetDomain string) (*external.GetAvatarURLResponse, error) {
	data, err := FedRpcRequest(r.cli, targetDomain, r.cfg.Rpc.FedAvatarTopic, req)
	if err != nil {
		log.Infof("federation rpc request failed, err: %v", err)
		return nil, err
	}

	var response external.GetAvatarURLResponse
	json.Unmarshal(data, &response)
	return &response, nil
}
...
func (r *Client) GetDisplayNameFromRemote(ctx context.Context, req *external.GetProfileRequest, targetDomain string) (*external.GetDisplayNameResponse, error) {
	data, err := FedRpcRequest(r.cli, targetDomain, r.cfg.Rpc.FedDisplayNameTopic, req)
	if err != nil {
		log.Infof("federation rpc request failed, err: %v", err)
		return nil, err
	}

	var response external.GetDisplayNameResponse
	json.Unmarshal(data, &response)
	return &response, nil
}
...
func (r *Client) GetRoomStateFromRemote(ctx context.Context, req *external.GetFedRoomStateRequest, targetDomain string) (*gomatrixserverlib.RespState, error) {
	data, err := FedRpcRequest(r.cli, targetDomain, r.cfg.Rpc.FedRsQryTopic, req)
	if err != nil {
		log.Infof("federation rpc request failed, err: %v", err)
		return nil, err
	}

	var response gomatrixserverlib.RespState
	json.Unmarshal(data, &response)
	return &response, nil
}
...
func (r *Client) DownloadFromRemote(ctx context.Context, req *external.GetFedDownloadRequest, targetDomain string) (io.Reader, int, http.Header, error) {
	...
	cb := &DownloadCB{id: req.ID, ch: ch}
	r.cli.SubRaw(cb)
	data, err := FedRpcRequest(r.cli, targetDomain, r.cfg.Rpc.FedDownloadTopic, req)
	if err != nil {
		log.Errorf("federation download rpc error: %v", err)
		r.cli.Unsubscribe(cb)
		close(ch)
		return nil, 500, nil, err
	}

	reader := &DownloadReader{ch: ch, cb: cb, rpcCli: r.cli}
...
func (r *Client) GetUserInfoFromRemote(ctx context.Context, req *external.GetUserInfoRequest, targetDomain string) (*external.GetUserInfoResponse, error) {
	data, err := FedRpcRequest(r.cli, targetDomain, r.cfg.Rpc.FedUserInfoTopic, req)
	if err != nil {
		log.Infof("federation rpc request failed, err: %v", err)
		return nil, err
	}

	var response external.GetUserInfoResponse
	json.Unmarshal(data, &response)
	return &response, nil
}
...
func (r *Client) MakeJoinToRemote(ctx context.Context, req *external.GetMakeJoinRequest, targetDomain string) (*gomatrixserverlib.RespMakeJoin, error) {
	data, err := FedRpcRequest(r.cli, targetDomain, r.cfg.Rpc.FedMakeJoinTopic, req)
	if err != nil {
		log.Errorf("federation make join error: %v", err)
		return nil, err
	}
	var response gomatrixserverlib.RespMakeJoin
	json.Unmarshal(data, &response)
	return &response, nil
}
...
func (r *Client) SendJoinToRemote(ctx context.Context, req *external.PutSendJoinRequest, targetDomain string) (*gomatrixserverlib.RespSendJoin, error) {
	data, err := FedRpcRequest(r.cli, targetDomain, r.cfg.Rpc.FedSendJoinTopic, req)
	if err != nil {
...
func (r *Client) MakeLeaveToRemote(ctx context.Context, req *external.GetMakeLeaveRequest, targetDomain string) (*gomatrixserverlib.RespMakeLeave, error) {
	data, err := FedRpcRequest(r.cli, targetDomain, r.cfg.Rpc.FedMakeLeaveTopic, req)
	if err != nil {
		log.Errorf("federation make leave error: %v", err)
		return nil, err
	}
	var response gomatrixserverlib.RespMakeLeave
	json.Unmarshal(data, &response)
	return &response, nil
}
...
func (r *Client) SendLeaveToRemote(ctx context.Context, req *external.PutSendLeaveRequest, targetDomain string) (*gomatrixserverlib.RespSendLeave, error) {
	data, err := FedRpcRequest(r.cli, targetDomain, r.cfg.Rpc.FedSendLeaveTopic, req)
	if err != nil {
...
func (r *Client) SendInviteToRemote(ctx context.Context, event *gomatrixserverlib.Event, targetDomain string) (*gomatrixserverlib.RespInvite, error) {
	data, err := FedRpcRequest(r.cli, targetDomain, r.cfg.Rpc.FedInviteTopic, event)
	if err != nil {
		log.Infof("federation rpc request failed, err: %v", err)
		return nil, err
	}

	var response gomatrixserverlib.RespInvite
	json.Unmarshal(data, &response)
	return &response, nil
}
