...
func (pd *PushDataRepo) AddPusher(ctx context.Context, pusher pushapitypes.Pusher, writeDB bool) error {
	...
	if writeDB {
		return pd.persist.AddPusher(ctx, pusher.UserName, pusher.ProfileTag, pusher.Kind, pusher.AppId, pusher.AppDisplayName, pusher.DeviceDisplayName, pusher.PushKey, pusher.PushKeyTs, pusher.Lang, []byte(pusher.Data), pusher.DeviceID)
	} else {
		return nil
	}
}
