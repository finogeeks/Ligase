...
func Send(msg *model.GobMessage, cache service.Cache, rpcCli roomserverapi.RoomserverRPCAPI, fedClient *client.FedClientWrap, db fedmodel.FederationDatabase) (*model.GobMessage, error) {
	...
	if len(pdus) > 0 {
		...
		if ev.Type() == gomatrixserverlib.MRoomCreate {
			...
			joinedRoomsVal, err = joinRoomsRepo.GetData(roomID)
			if err != nil {
		...
		joinedRoomsVal, _ := joinRoomsRepo.GetData(roomID)
		...
		if eventOffset == nil || eventOffset.Offset == 0 {
			idMap, _ := backfillRepo.GetFinishedDomains(roomID)
			if idMap != nil {
				if v, ok := idMap[domain]; ok {
					eventOffset = &repos.JoinedRoomFinishedEventOffset{v.StartEventID, v.StartOffset}
				}
			}
		}
		...
		getMissingEvents(eventOffset, domain, pdus)
		...
		rpcCli.InputRoomEvents(context.TODO(), &rawEvent)
		...
		joinRoomsRepo.UpdateData(joinedRoomsVal)
	}
...
func getMissingEvents(eventOffset *repos.JoinedRoomFinishedEventOffset, domain string, pdus []gomatrixserverlib.Event) {
	sort.Slice(pdus, func(i, j int) bool { return pdus[i].DomainOffset() < pdus[j].DomainOffset() })
	...
	if eventOffset.Offset != 0 && eventOffset.Offset != pdus[0].DomainOffset()-1 && eventOffset.Offset < pdus[0].DomainOffset() {
		...
		backfillEvents(roomID, pdus[0].EventID(), limit, pdus[0].DomainOffset())
	}
	...
	for i := 1; i < len(pdus); i++ {
		...
		if v0.DomainOffset() != v1.DomainOffset()-1 && v0.DomainOffset() < v1.DomainOffset() {
			...
			backfillEvents(roomID, v1.EventID(), limit, v1.DomainOffset())
		}
	}
}

func backfillEvents(roomID, eventID string, limit int, domainOffset int64) error {
