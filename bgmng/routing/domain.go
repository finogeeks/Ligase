...
func OnUpdateServerNameCfg(
	...
	if req.Type == "add" {
		OnAddServerNames(cache, serverConfDB, req.ServerNames, idg)
	} else if req.Type == "del" {
		OnDelServerNames(cache, serverConfDB, req.ServerNames)
	} else {
...
func OnAddServerNames(cache service.Cache, serverConfDB model.ConfigDatabase, serverNames []string, idg *uid.UidGenerator) {
	for _, serverName := range serverNames {
		...
		err = serverConfDB.UpsertServerName(context.Background(), nid, serverName)
		if err != nil {
...
//current has not del
func OnDelServerNames(cache service.Cache, serverConfDB model.ConfigDatabase, serverNames []string) {
...
func getServerNameCfg(
	...
	if err != nil || len(domains) <= 0 {
		...
		domains, err = serverConfDB.SelectServerNames(context.Background())
		if err != nil {
