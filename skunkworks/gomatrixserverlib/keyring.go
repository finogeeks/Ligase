...
func (k KeyRing) VerifyJSONs(ctx context.Context, requests []VerifyJSONRequest) ([]VerifyJSONResult, error) { // nolint: gocyclo
	...
	for _, fetcher := range k.KeyFetchers {
		...
		fetcherFields := append(fields, log.KeysAndValues{"fetcher", fetcher.FetcherName()}...)

		// TODO: Coalesce in-flight requests for the same keys.
		// Otherwise we risk spamming the servers we query the keys from.

		reqFetcherFields := append(fetcherFields, log.KeysAndValues{"num_key_requests", len(keyRequests)}...)
		log.Infow("Requesting keys from fetcher", reqFetcherFields)
		...
		fetchedFetcherFields := append(fetcherFields, log.KeysAndValues{"num_keys_fetched", len(keysFetched)}...)
		log.Infow("Got keys from fetcher", fetchedFetcherFields)
