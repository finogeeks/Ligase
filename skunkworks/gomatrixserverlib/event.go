...
func (eb *EventBuilder) Build(eventNID int64, now time.Time, origin ServerName) (result Event, err error) {
	result.fields.RoomID = eb.RoomID
	result.fields.EventID = fmt.Sprintf("$%d:%s", eventNID, origin)
	result.fields.EventNID = eventNID
	result.fields.Sender = eb.Sender
	result.fields.Type = eb.Type
	result.fields.StateKey = eb.StateKey
	result.fields.Content = eb.Content
	result.fields.Redacts = eb.Redacts
	result.fields.Depth = eb.Depth
	result.fields.DomainOffset = eb.DomainOffset
	result.fields.Unsigned = eb.Unsigned
	result.fields.OriginServerTS = 0
...
func (e Event) External() bool { return e.external }

func (e Event) SetExternal(external bool) { e.external = external }

func (e *Event) SetRedactEventSender(sender string) { e.fields.RedactsSender = sender }

func (e *Event) RedactEventSender() string { return e.fields.RedactsSender }
...
func (e Event) SetUnsigned(unsigned interface{}, resetContent bool) (Event, error) {
	...
	var err error
	if err = json.Unmarshal(e.JSON(), &eventAsMap); err != nil {
		return Event{}, err
	}
	...
	if resetContent {
		...
		eventAsMap["content"] = empty
		e.fields.Content = empty
	}
...
func (e *Event) SetUnsignedField(path string, value interface{}) error {
	...
	path = "unsigned." + path
	eventJSON, err := sjson.SetBytes(e.JSON(), path, value)
	if err != nil {
		return err
	}
	eventJSON = CanonicalJSONAssumeValid(eventJSON)
...
func (e Event) EventReference() EventReference {
	return EventReference{e.EventID(), []byte{}}
	/*
		reference, err := referenceOfEvent(e.JSON())
		if err != nil {
			...
			panic(fmt.Errorf("gomatrixserverlib: invalid event %v (%q)", err, string(e.JSON())))
		}
		return reference
	*/
