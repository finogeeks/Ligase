...
var skipMap sync.Map
var skipMapConf sync.Map

func AddSkipItem(patten string, isReg bool) {
	//configOnce.Do(func() {
	//	skipMap = new(sync.Map)
	//})

	skipMapConf.Store(patten, isReg)
}
...
func createEventAllowed(event Event, authEvents AuthEventProvider) error {
	create, _ := authEvents.Create()
	if create != nil {
		return errorf("create event must be the first event in the room: found %s prev_events", create.EventID())
	}
...
func memberEventAllowed(event Event, authEvents AuthEventProvider) error {
	...
	pre, _ := authEvents.Member(*event.StateKey())
	return allower.membershipAllowed(event, pre)
}
...
func redactEventAllowed(event Event, authEvents AuthEventProvider) error {
	...
	if senderDomain == redactDomain {
		if event.Sender() == event.fields.RedactsSender {
			return nil
		} else if event.Sender() == allower.create.Creator {
			return nil
		} else {
			return errorf(
				"%q is not allowed to redact message from %q send by %s create by %s",
				event.Sender(), redactDomain, event.RedactEventSender, allower.create.Creator,
			)
		}
	}
...
func (e *eventAllower) commonChecks(event Event) error {
	...
	skip := false
	if val, ok := skipMap.Load(event.Type()); ok {
		skip = val.(bool)
	} else {
		skipMapConf.Range(func(key, value interface{}) bool {
			patten := key.(string)
			isReg := value.(bool)

			if isReg == false {
				if patten == event.Type() {
					skip = true
					return false
				}
			} else {
				skip, _ = regexp.MatchString(patten, event.Type())
				if skip == true {
					return false
				}
			}
		...
		skipMap.Store(event.Type(), skip)
	}

	if skip == false {
		senderLevel := e.powerLevels.userLevel(sender)
		eventLevel := e.powerLevels.eventLevel(event.Type(), stateKey != nil)
		// compatibility old channel
		if senderLevel < eventLevel && event.Type() != MRoomArchive {
			return errorf(
...
func newMembershipAllower(authEvents AuthEventProvider, event Event) (m membershipAllower, err error) { // nolint: gocyclo
	...
	if m.newMember.Membership == "kick" {
		m.newMember.Membership = leave
	}
...
func (m *membershipAllower) membershipAllowed(event Event, prevEvent *Event) error { // nolint: gocyclo
	...
	//for auto join
	if m.newMember.Membership == join &&
		m.senderID == m.create.Creator &&
		m.targetID != m.create.Creator &&
		prevEvent != nil {
...
func (m *membershipAllower) membershipAllowedSelf() error { // nolint: gocyclo
	if m.newMember.Membership == join {
		...
		if m.oldMember.Membership == forget && m.joinRule.JoinRule == public {
			return nil
		}
	...
	if m.newMember.Membership == forget {
		// A joined user is allowed to leave the room.
...
func (m *membershipAllower) membershipAllowedOther() error { // nolint: gocyclo
	...
	if m.newMember.Membership == invite {
		...
		if m.oldMember.Membership == forget && senderLevel >= m.powerLevels.inviteLevel {
			return nil
		}
	}
