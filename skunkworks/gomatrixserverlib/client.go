...
func NewHttpsClient(rootCrt, certPem, keyPem string) *Client {
	...
	pool.AppendCertsFromPEM([]byte(rootCrt))

	cliCrt, err := tls.X509KeyPair([]byte(certPem), []byte(keyPem))
	if err != nil {
	...
	return NewHttpsClientWithTimeout(requestTimeout, tr)
}
...
func (f *federationTripper) RoundTrip(r *http.Request) (*http.Response, error) {
	...
	if len(dnsResult.Addrs) == 0 {
		e := gomatrix.HTTPError{
			WrappedError: fmt.Errorf("no address found for matrix host %v", serverName),
			Message:      fmt.Sprintf("no address found for matrix host %v", serverName),
			Code:         http.StatusNotFound,
		}
		return nil, e //fmt.Errorf("no address found for matrix host %v", serverName)
	}
	...
	for _, addr := range dnsResult.Addrs {
		...
		fields := util.GetLogFields(r.Context())
		log.Warnw(fmt.Sprintf("Error sending request to %s: %v", u.String(), err), fields)
	}
...
func (fc *Client) LookupUserInfo(
	...
	response, err = fc.DoHTTPRequest(ctx, req, false)
	if response != nil {
...
func (fc *Client) CreateMediaDownloadRequest(
	...
	return fc.DoHTTPRequest(ctx, req, true)
}
...
func (fc *Client) DoRequestAndParseResponse(
	...
	if response.StatusCode/100 != 2 { // not 2xx
		// Adapted from https://github.com/finogeeks/dendrite/skunkworks/gomatrix/blob/master/client.go
		var contents []byte
...
func (fc *Client) DoHTTPRequest(ctx context.Context, req *http.Request, noTimeout bool) (*http.Response, error) {
	...
	fields = append(fields, log.KeysAndValues{"server", req.URL.Host, "out.req.ID", reqID}...)
	newCtx := util.ContextWithLogFields(ctx, fields)

	log.Infow(fmt.Sprintf("Outgoing request %s %s", req.Method, req.URL), fields)
	cli := fc.client
	if noTimeout {
		cli = fc.noTimeoutCli
	}
	resp, err := cli.Do(req.WithContext(newCtx))
	if err != nil {
		log.Infow(fmt.Sprintf("Outgoing request %s %s failed with %v", req.Method, req.URL, err), fields)
		return nil, err
	}

	// we haven't yet read the body, so this is slightly premature, but it's the easiest place.
	log.Infow(fmt.Sprintf("Response %d from %s %s", resp.StatusCode, req.Method, req.URL), fields)
