...
func (r *FederationRequest) HTTPRequest(proto string) (*http.Request, error) {
	...
	urlStr := fmt.Sprintf("%s://%s%s", proto, r.fields.Destination, r.fields.RequestURI)
	...
	*/
	...
	httpReq.Header.Add("Authorization", fmt.Sprintf(
		"X-Matrix origin=\"%s\",key=\"%s\",sig=\"%s\"", r.fields.Origin, "emptykey", "emptysig",
	))
...
func VerifyHTTPRequest(
	...
	if err != nil {
		...
		log.Printw("Error parsing HTTP headers", fields)
	...
	if err != nil {
		fields := util.GetLogFields(req.Context())
		fields = append(fields, log.KeysAndValues{"error", err}...)
		log.Printw("Error parsing JSON", fields)
	...
	if request.Origin() == "" {
		...
		fields := util.GetLogFields(req.Context())
		fields = append(fields, log.KeysAndValues{"error", err}...)
		log.Printw(message, fields)
		return nil, util.MessageResponse(401, message)
	}

	// do not verify keys when it is nil
	if keys != nil {
